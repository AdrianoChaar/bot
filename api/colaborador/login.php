<?php
    ...
    include_once '../config/core.php'; include_once '../config/database.php'; include_once '../objetos/colaborador.php';
    $database=new Database(); $db=$database->getConnection(); $colaborador=new Colaborador($db); 
    $keywords=isset($_GET["s"]) ? $_GET["s"] : ""; $stmt=$colaborador->login($keywords); $num=$stmt->rowCount();
	if($num>0){$colaborador_arr=array(); $colaborador_arr["registros"]=array();
		while($row=$stmt->fetch(PDO::FETCH_ASSOC)){extract($row);
            $colaborador_item=array("id"=>$id,"nome"=>$nome, "login"=>html_entity_decode($login), "senha"=>$senha);
